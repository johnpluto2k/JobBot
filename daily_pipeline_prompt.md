...
3. Delegate to outreach-drafter for any follow-ups that are now due
   ...
   Expect back: what was drafted (status='drafted') and who the candidate must review/send
   manually.
...
3.5. Delegate to inbox-triager: classify unhandled recruiter email
   ...
   live opportunities and scheduling requests that need the candidate's action, plus bare
...
5. Write a short summary (not a full data dump) to
   ...
   clearly-marked **"Needs the candidate's attention"** section at the top — anything
   ...
   career-coach skill / CLAUDE.md coaching mode references when the candidate asks "what
   happened while I was away," so lead with the time-sensitive items.

6. **File the Agent HQ report** (the owner's Obsidian dashboard; skip this step if you
   do not keep an Obsidian vault, and replace `<your Obsidian vault>` with its path). Do this yourself
   (conductor), last, even if earlier steps failed — a failure report is still a
   report:
   - Write `<your Obsidian vault>\Reports\<YYYY-MM-DD> Job Bot.md`
     ...
     **Needs the candidate:** no — nothing to review  *(or: yes — one bullet per item)*
   ...
   - Then update the **Job Bot row only** of the "Team status" table in
     `<your Obsidian vault>\1 Projects\Agent HQ.md` to
...
7. **Sync the Obsidian vault** (conductor, right after the Agent HQ report; same
   ...
   a. **Daily note — Pipeline section.** Open
      `<your Obsidian vault>\Daily Notes\<YYYY-MM-DD>.md`. If it
      doesn't exist, create it from
      `<your Obsidian vault>\Templates\Daily Note.md` (replace
      ...
      - drafts awaiting the candidate's review,
   ...
   b. **Rolling funnel snapshot.** Run `python3 coach_snapshot.py .` and
      **overwrite** (full rewrite, newest data only)
      `<your Obsidian vault>\2 Areas\Job Search Pipeline Status.md`
      with this shape:
